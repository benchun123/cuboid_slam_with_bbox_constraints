...
bool whether_use_only_first_pose;
		       ...
		       std::vector<object_landmark*> all_frame_rawcubes, Eigen::MatrixXd& truth_frame_poses)
{
	...
	string resultsPath = dataset_folder + "output_cam_poses.txt";
	...
	string objresultsPath = dataset_folder + "output_obj_poses.txt";
	...
	if (frame_number<total_frame_number)
	{
	    ...
	    if (all_frame_rawcubes.size()>0 && all_frame_rawcubes[frame_number]!=nullptr)
			...
			// pub_frame_raw_cube.publish(cuboids_to_marker(all_frame_rawcubes[addnumber],Vector3d(0,0,1)));
		ros::Duration(0.2).sleep(); // just slow down
...
// 			  all_pred_pose_odoms[frame_number].pose.pose.position.y <<std::endl;
	    ...
	    std::string raw_rgb_img_name = dataset_folder+"raw_imgs/" + std::string(frame_index_c) + "_rgb_raw.jpg";
		...
		{
		    std::string raw_depth_img_name = dataset_folder+"depth_imgs/" + std::string(frame_index_c) + "_depth_raw.png";
...
{  
	...
	int total_frame_number = int(addnumber);
    ...
    detect_cuboid_obj.max_cuboid_num = addnumber;
    ...
    for (int frame_index=0;frame_index<total_frame_number;frame_index++)
    {
	  ...
	  {
		//start detect cuboid
		cv::Mat raw_rgb_img = cv::imread(dataset_folder+"raw_imgs/"+frame_index_c+"_rgb_raw.jpg", 1);
		// std::cout << "frame_index: " << frame_index << std::endl;
		...
		raw_2d_objs.setZero();
		if (!read_all_number_txt(dataset_folder+"/bbox_2d_yolov2/"+frame_index_c+"_yolo2_0.15.txt", raw_2d_objs))
		return;
		...
		detect_cuboid_obj.detect_cuboid_new(raw_rgb_img,transToWolrd,raw_2d_objs,all_lines_raw, frames_cuboids);
		// read offline saved 2d image
		std::string detected_cube_2d_img_name = dataset_folder+"/best_objects_img/" + std::string(frame_index_c) + "_best_objects.jpg";
		...
		if (has_detected_cuboid)  // prepare object measurement
		{
			...
			proposal_error = 0.222742;

			// std::cout << "detected cube" << cube_ground_value.toMinimalVector().transpose() << std::endl;
			// std::cout << "former twc " << curr_cam_pose_Twc.toMinimalVector().transpose() << std::endl;
			// std::cout << "detected_cube->camera_roll_delta " << detected_cube->camera_roll_delta 
			// << " "<< detected_cube->camera_pitch_delta << std::endl;
			...
			if (detect_cuboid_obj.whether_sample_cam_roll_pitch)  //if camera roll/pitch is sampled, transform to the correct camera frame.
			{
			  ...
			  quat_to_euler_zyx(Quaterniond(Rot_Mat), new_camera_eulers(0), new_camera_eulers(1), new_camera_eulers(2));
		      ...
		      cube_local_meas = cube_ground_value.transform_to(curr_cam_pose_Twc_new);
				// std::cout << "new_camera_eulers: " << new_camera_eulers.transpose() << std::endl;
				std::cout << "curr_cam_pose_Twc_new: " << curr_cam_pose_Twc_new.toVector().transpose() << std::endl;
	  ...
	  {
		// // // read ground-based value together with fix camera, change to local value 
		if(whether_use_only_first_pose)
		{
			...
			if (has_detected_cuboid)  // prepare object measurement   not all frame has observation!!
			{
				...
				// transToWolrd.block(0,0,3,3) = Quaterniond(cam_pose_vec(7),cam_pose_vec(4),cam_pose_vec(5),cam_pose_vec(6)).toRotationMatrix();
				// transToWolrd.col(3).head(3) = Eigen::Vector3d(cam_pose_vec(1), cam_pose_vec(2), cam_pose_vec(3));
				// std::cout << "transToWolrd: \n" << transToWolrd << std::endl;
		...
		else	// // read local values together with fix camera 
		{
			int cube_obs_frame_id = offline_pred_frame_objects(offline_cube_obs_row_id,0);
			has_detected_cuboid = cube_obs_frame_id==frame_index;
			if (has_detected_cuboid)  // prepare object measurement   not all frame has observation!!
...
int main(int argc,char* argv[])
{
    ...
    nh.param ("/base_folder", base_folder, ros::package::getPath("object_slam_bbox")+"/data/");
    nh.param ("/online_detect_mode", online_detect_mode, false);
    nh.param ("/whether_use_only_first_pose", whether_use_only_first_pose, false);
    nh.param ("/save_results_to_txt", save_results_to_txt, false);
    nh.param ("/addnumber", addnumber, 0.0);

	base_folder = base_folder.substr(0, base_folder.find_last_of("/"));
	base_folder = base_folder.substr(0, base_folder.find_last_of("/"));
	base_folder = base_folder.substr(0, base_folder.find_last_of("/"));
	dataset_folder = base_folder + "/data/";
    cout<<"dataset_folder   "<<dataset_folder<<endl;
    if (online_detect_mode)
    ...
    std::string pred_objs_txt = dataset_folder+"online_cubes.txt";  // saved cuboids in local or ground frame.
    std::string init_camera_pose = dataset_folder+"online_camera.txt"; // offline camera pose for cuboids detection (x y yaw=0, truth roll/pitch/height)
    std::string truth_objs_txt = dataset_folder+"truth_objects.txt";  // saved ground truth in global frame
    std::string truth_camera_pose = dataset_folder+"truth_cam_poses.txt";
